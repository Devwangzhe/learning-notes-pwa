...
const Store = {
    ...
    // 普通方法（避免与 getter 语法冲突）
    getData(key) {
        try {
            const raw = localStorage.getItem('shendu_' + key);
            return raw ? JSON.parse(raw) : JSON.parse(JSON.stringify(this._defaults[key]));
        } catch(e) { return JSON.parse(JSON.stringify(this._defaults[key])); }
    },
    setData(key, val) { localStorage.setItem('shendu_' + key, JSON.stringify(val)); },

    // Getter/Setter 属性
    get notes() { return this.getData('notes'); },
    set notes(v) { this.setData('notes', v); },
    get bookshelf() { return this.getData('bookshelf'); },
    set bookshelf(v) { this.setData('bookshelf', v); },
    get settings() { return this.getData('settings'); },
    set settings(v) { this.setData('settings', v); },
};
...
async function startAnalysis() {
    ...
    const _pf = document.getElementById('overlay-progress-fill'); if (_pf) _pf.style.width = '0%';
